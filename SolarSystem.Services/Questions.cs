...
namespace SolarSystem.Services
{
    public class Questions
    {
        ...
        {
            return _Db.GetAllItemsOfTypePlanet()
                .Where(i => i.Name.ToUpper().Contains("P") || i.Name.ToUpper().Contains("T")).Select(i => i.Name)
                .ToList();
        }
        ...
        {
            return _Db.GetAllItemsOfTypePlanet().Where(i => i.Class == Planet.TypeOfPlanet.DwarfPlanet)
                .OrderBy(i => i.KnownMoons).Select(i => i.Name).ToList();
        }

        public short Question8()
        {
            short amountOfMoons = 0;
            foreach (var item in _Db.GetAllItemsOfTypePlanet()) amountOfMoons += item.KnownMoons;
        ...
        {
            return _Db.GetAllItemsOfTypePlanet().Where(i => i.Class == Planet.TypeOfPlanet.DwarfPlanet)
                .OrderBy(i => i.Diameter).Select(i => i.Name).ToList();
        }
